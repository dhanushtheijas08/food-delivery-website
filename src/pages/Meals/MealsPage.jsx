...
function MealsPage() {
  const mealsCardRender = mealsCardData.map((menu) => (
    <MealsCard foodName={menu.foodName} foodImgPath={menu.foodImgPath} />
  ));
  const dietListRender = dietList.map((item) => (
    <IconWithText iconPath={item.iconPath} text={item.name} iconSize="h-5" />
  ));
  return (
    <div className="py-10 px-10 text-textDark">
      <SectionHeading
        sectionHeading="MEALS"
        mainHeading="Well Menu AI chooses from 5,000+ recipes"
        classname="text-center"
      />
      <div className="flex justify-around mt-10 flex-col md:flex-row space-y-10 md:space-y-0">
        {mealsCardRender}
        ...
        <div className="hidden lg:block">
          ...
          <div className="flex flex-col space-y-4 mt-4 ">{dietListRender}</div>
        </div>
      </div>
    </div>
  );
